...
if (!neonUrl || neonUrl.includes('username:password@ep-sample-pooler')) {
  console.error('❌ Error: NEON_DATABASE_URL is not configured in backend/.env.');
  console.error('👉 Please paste your real Neon connection string into NEON_DATABASE_URL in backend/.env first.');
  process.exit(1);
}

// Connect strictly to Neon Cloud Database
...
async function main() {
  console.log('🌱 Connecting strictly to NEON PostgreSQL Cloud Database...');
  console.log('📡 Neon Target URL:', neonUrl?.replace(/:[^:@]+@/, ':****@'));

  // 1. Clean existing records in reverse relation order for clean idempotency in Neon
  ...
  console.log('🧹 Cleaned existing records in Neon.');

  // 2. Create Role-based Users (Ravindra Kumar as Admin)
  ...
  console.log('👤 Created 4 demo users in Neon: Admin (Ravindra Kumar), Sales, Warehouse, Accounts.');
  ...
  console.log(`🏢 Seeded ${createdCustomers.length} B2B customers in Neon.`);
  ...
  console.log(`📋 Seeded ${followUpData.length} CRM follow-up records in Neon.`);
  ...
  console.log(`📦 Seeded ${createdProducts.length} product items in Neon.`);
  ...
  console.log(`📊 Seeded ${stockMovementData.length} stock movement records in Neon.`);
  ...
  console.log(`📄 Seeded 4 delivery challans with item snapshot lines in Neon.`);
